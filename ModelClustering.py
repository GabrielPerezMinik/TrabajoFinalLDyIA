...
def preprocess_batch_type1(chunk, batch_number=None):
    ...
    processed = apply_clustering(chunk, scaler=MinMaxScaler(), eps=0.5, min_samples=5)
    return processed
...
def preprocess_batch_type2(chunk, batch_number):
    ...
    clustered_features = apply_clustering(pd.DataFrame(features, columns=features.columns),
...
if __name__ == "__main__":
    ...
    data_pca = apply_pca(df_normalized)
    distances = calculate_neighbors(data_pca)
    plot_distances(distances)

    # --- Step 2: Batch processing using Type 1 method ---
    final_df_type1 = process_batches(filepath, batch_size=50000, preprocess_func=preprocess_batch_type1,
                                     encoding='iso-8859-1')
    save_results(final_df_type1, output_file="ClusterData/dbscan_batches_result_type1.csv")
    plot_clusters(final_df_type1)

    # --- Step 3: Batch processing using Type 2 method ---
    try:
        final_df_type2 = process_batches(filepath, batch_size=50000, preprocess_func=preprocess_batch_type2,
                                         encoding='unicode_escape')
        analyze_clustering_results(final_df_type2)
