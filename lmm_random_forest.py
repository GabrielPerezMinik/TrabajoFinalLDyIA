...
def train_and_evaluate_models(df_daily_train_robust, df_daily_val_robust, df_daily_test_robust):
    ...
    for scaler, train_data, val_data, test_data in [
        ...
        results.append({
            ...
            'r2_test': evaluate_model(best_model, pd.concat([train_data, val_data]), test_data),
            'mae_test': evaluate_model(best_model, pd.concat([train_data, val_data]), test_data),
            'best_parameters': grid_search.best_params_,
            'cv_rmse': cv_rmse
        })

        # Create comparison charts
        create_comparison_graphs(best_model, test_data)
...
def create_comparison_graphs(best_model, test_data):
    ...
    plt.bar(['Random Forest'], [mae_rf], color=['blue', 'green'])
